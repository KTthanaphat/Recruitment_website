...
export function PipelineFunnel({
  ...
  return (
    <section className="pipeline-funnel min-w-0 rounded-lg border border-[#D7DEE8] bg-white p-4 shadow-[0_8px_20px_rgba(11,19,43,0.03)]">
              ...
              <div key={row.key} className="contents">
                ...
                <div className={`border-t border-l border-[#D7DEE8] px-3 py-2 ${isApplicantRow ? "bg-white" : "bg-[#F8FBFF]"}`}>
                  <div className="relative min-h-9 overflow-hidden rounded-r-md bg-[#EEF2F7]" aria-label={`${row.label}: ${formatNumber(row.count, language)}`}>
                    <div
                      className={`absolute inset-y-0 left-0 rounded-r-md ${active ? "bg-primary" : "bg-[#D7DEE8]"} ${isApplicantRow ? "bg-[#0A3CDC]" : ""}`}
