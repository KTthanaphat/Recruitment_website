  ...
  return (
    <div className={compact ? "relative z-0 grid gap-1.5" : "relative z-0 rounded-lg bg-white px-4 py-4"}>
      ...
      <ol aria-label={ariaLabel ?? label ?? "Candidate pipeline journey"} className={compact ? "relative z-0 flex items-center gap-1.5" : "relative z-0 grid gap-4 md:grid-cols-7 md:gap-2 md:justify-items-center"}>
        {!compact ? <span className="absolute left-[8.5%] right-[8.5%] top-3 z-0 hidden h-0.5 bg-[#D7DEE8] md:block" /> : null}
          ...
          return (
            ...
            >
              {index < PIPELINE_JOURNEY_STAGES.length - 1 ? (
                <span className="absolute left-3 top-6 h-[calc(100%+1rem)] w-0.5 bg-[#D7DEE8] md:hidden" />
              ) : null}
              ...
              <div className="min-w-0 md:mt-3 md:flex md:min-h-[72px] md:flex-col md:items-center md:justify-start">
                <p className="text-xs font-semibold leading-tight text-navy">{pipelineDisplayLabel(stage)}</p>
...
function stageDotClass(state: StageRailState, current: boolean) {
  if (state === "passed") return current ? "bg-primary ring-[#C9D5E6] text-white" : "bg-primary ring-[#EEF4FF] text-white";
  ...
  return "bg-[#D7DEE8] ring-[#F6F8FC] text-slate";
}
