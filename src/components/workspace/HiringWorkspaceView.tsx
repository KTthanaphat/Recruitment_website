...
import { formatDate, formatRequisitionCardTitle, formatRequisitionOptionLabel, formatRequisitionTitle, resultText, statusTone } from "@/lib/format";
...
export function HiringWorkspaceView({
  ...
  return (
    <div className="grid min-w-0 gap-5">
      ...
      {pickerOpen || !context ? (
        <WorkspacePicker candidates={candidates} canCreateRequisition={canWrite && Boolean(onDispatchAction)} canManageSetup={canManageSetup && Boolean(onDispatchAction)} groups={activeOpenGroups} invalidTarget={Boolean(selectedTarget.type && selectedTarget.id && !context)} language={language} mode={pickerMode} onCreateGroup={() => onDispatchAction?.({ kind: "group.create" })} onCreateRequisition={() => onDispatchAction?.({ kind: "requisition.create" })} onModeChange={setPickerMode} onSelect={selectTarget} requisitions={activeOpenRequisitions} />
      ) : (
...
function WorkspacePicker({ candidates, canCreateRequisition, canManageSetup, groups, invalidTarget, language, mode, onCreateGroup, onCreateRequisition, onModeChange, onSelect, requisitions }: { candidates: EnrichedCandidate[]; canCreateRequisition: boolean; canManageSetup: boolean; groups: EnrichedSourcingGroup[]; invalidTarget: boolean; language: Language; mode: PickerMode; onCreateGroup: () => void; onCreateRequisition: () => void; onModeChange: (mode: PickerMode) => void; onSelect: (target: SelectedWorkspaceTarget) => void; requisitions: EnrichedRequisition[] }) {
    ...
    <Panel className="min-w-0">
      ...
      <div className="mt-3 grid min-w-0 gap-2 sm:grid-cols-2 xl:grid-cols-3">
        ...
        {mode === "requisitions" ? (rows as EnrichedRequisition[]).map((row) => {
          ...
          return <button key={row.doc_id} type="button" aria-label={formatRequisitionOptionLabel(row)} className="ats-card grid min-w-0 gap-1.5 p-3 text-left" onClick={() => onSelect({ type: "requisition", id: row.doc_id })}><div className="flex min-w-0 items-start justify-between gap-2"><strong className="break-words text-navy">{formatRequisitionCardTitle(row)}</strong><Tag tone={row.open_headcount > 0 ? "warning" : "success"}>{translate(language, "openCount", { count: row.open_headcount })}</Tag></div><p className="text-xs font-medium text-cool">{translate(language, "requisitionId")}: {row.doc_id}</p><p className="text-xs font-medium text-cool">{row.site} - {row.person_in_charge ?? "-"} - {translate(language, "candidatesCount", { count: candidateCount })}</p></button>;
        }) : (rows as EnrichedSourcingGroup[]).map((group) => <button key={group.group_id} type="button" className="ats-card grid min-w-0 gap-2 p-3 text-left" onClick={() => onSelect({ type: "group", id: group.group_id })}><div className="flex min-w-0 items-start justify-between gap-2"><strong className="block min-w-0 flex-1 truncate text-navy" title={group.group_position}>{group.group_position}</strong><Tag tone="muted">{translate(language, "candidatesCount", { count: group.candidate_count })}</Tag></div><p className="text-xs font-semibold text-primary">{translate(language, "groupId")}: {group.group_id}</p><p className="text-xs font-medium text-cool">{group.sites.join(", ")} - {group.owners.join(", ") || "-"}</p></button>)}
      </div>
    </Panel>
  );
}
