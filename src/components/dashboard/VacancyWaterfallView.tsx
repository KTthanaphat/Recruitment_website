...
export function VacancyWaterfallView({
  ...
  const reportSummary = useMemo(() => buildReportSummary(requisitionRows, funnelRows), [funnelRows, requisitionRows]);
  ...
  async function exportRequisitionDetailXlsx() {
    ...
    try {
      ...
      const metadata = XLSX.utils.json_to_sheet([{
        ...
        "Generated By": data.profile?.email ?? data.profile?.nickname ?? "Unknown",
        "Date Range": `${startDate} to ${endDate}`,
        "Rows": requisitionRows.length
      }]);
      const workbook = XLSX.utils.book_new();
      XLSX.utils.book_append_sheet(workbook, worksheet, "Opened Requisitions");
      XLSX.utils.book_append_sheet(workbook, metadata, "Export Metadata");
    ...
    <div className="grid min-w-0 max-w-full gap-4 overflow-x-hidden">
      <section className="min-w-0 max-w-full overflow-hidden rounded-lg border border-[#D7DEE8] bg-white py-6 font-normal shadow-[0_8px_22px_rgba(11,19,43,0.035)]">
        <div className="mb-5 grid gap-5 border-b border-[#D7DEE8] px-4 pb-5 sm:px-6 lg:grid-cols-[1fr_auto] lg:items-start lg:px-8">
          ...
          <div className="grid gap-3 rounded-md bg-[#F8FAFD] p-3 sm:flex sm:items-start sm:justify-end">
            ...
            <div className="flex flex-wrap items-end gap-2 pt-5 print:hidden">
              <Button type="button" size="sm" variant="secondary" onClick={() => applyReportPreset("weekly")}>Weekly</Button>
              <Button type="button" size="sm" variant="secondary" onClick={() => applyReportPreset("monthly")}>Monthly</Button>
              <Button type="button" size="sm" variant="secondary" onClick={() => applyReportPreset("site")}>Site Review</Button>
        ...
        <div className="mb-5 px-4 sm:px-6 lg:px-8">
          <OperationalSummaryStrip items={reportSummary} />
        </div>
      ...
      <section className="min-w-0 max-w-full overflow-hidden rounded-lg border border-[#D7DEE8] bg-white shadow-[0_8px_22px_rgba(11,19,43,0.035)]">
        ...
        {funnelOpen ? (
          <div className="grid min-w-0 gap-4 border-t border-[#D7DEE8] p-4 sm:p-6 lg:p-8">
            ...
            />
            <p className="text-sm font-medium text-slate">{funnelApplicantTotal === 0 ? "No applicants match the selected funnel filters." : `Top bottleneck: ${topFunnelBottleneck(funnelRows)}.`}</p>
          </div>
        ) : null}
      </section>
      ...
      <div data-print-section="pipeline-funnel" className="print-report-only print-funnel-report">
        ...
        <p className="px-4 pb-3 text-sm font-medium text-slate sm:px-6 lg:px-8">Level: {funnelLevelLabel(funnelLevelBand)} - Channel: {funnelChannelLabel}</p>
        <PipelineFunnel
          language={language}
          rows={funnelRows}
          title="Recruitment Pipeline Health"
...
function buildReportSummary(requisitionRows: RequisitionDetailRow[], funnelRows: PipelineFunnelRow[]) {
  ...
  const bottleneck = topFunnelBottleneck(funnelRows);
  return [
    { label: "Opened", value: opened, tone: "primary" as const, helper: "Requisitions in range" },
    { label: "Filled", value: filled, tone: filled > 0 ? "success" as const : "muted" as const, helper: "Filled requisitions" },
    { label: "Open HC", value: openHeadcount, tone: openHeadcount > 0 ? "warning" as const : "success" as const, helper: "Remaining vacancy" },
    { label: "Over SLA", value: overSla, tone: overSla > 0 ? "danger" as const : "success" as const, helper: "Needs review" },
    { label: "Bottleneck", value: bottleneck, tone: "teal" as const, helper: "Largest funnel drop" }
  ];
}

function topFunnelBottleneck(rows: PipelineFunnelRow[]) {
  ...
  return top && top.drop > 0 ? `${top.label} (-${top.drop})` : "No major drop";
}
