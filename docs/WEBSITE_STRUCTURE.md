...
- An admin Outcome correction may update only the immediate next unresolved Pending date. It is rejected once downstream completed history exists or when the new Pending date would exceed that Pending’s estimate; historical records are never shifted.
