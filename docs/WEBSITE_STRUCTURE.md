...
The Recruitment Calendar opens on the current Bangkok month and keeps Previous, Next, and Today navigation in local Home state. At normal desktop width, Today’s Work ends at the visible Calendar grid in the same 1:3 row and its priority list scrolls internally; narrower screens stack them. Desktop uses 96px Sunday-first day cells with the date top-left and the `(N)` count top-right, aligned on one header row. Event eligibility and accessible detail-drawer behavior remain unchanged.
...
Active-requisition detail is collapsed by default. It includes each non-cancelled requisition with a valid PR date on or before the selected end date and no filled-status date before the selected start date; every eligible row contributes original headcount. The single Status field uses the same period-end requisition snapshot as the Vacancy Waterfall: the latest requisition status log on or before the selected end date. Filled Date is the latest filled-status date on or before that same end date and is blank unless Status is Filled. The Waterfall chart remains a separate movement report. Detail includes ownership, requisition date, applicant totals, historical pipeline stage counts, SLA status, the unified historical status, and fill date.
