-- Candidate email is optional and intentionally non-unique.
...
begin
  ...
  if v_email is not null and v_email !~ '^[^[:space:]@]+@[^[:space:]@]+\.[^[:space:]@]+$' then raise exception 'CANDIDATE_EMAIL_INVALID: Enter a valid email address.'; end if;
  ...
  values (v_candidate_id, nullif(payload ->> 'name', ''), v_nickname, v_phone_no, v_email, v_doc_group_id, v_group_id, nullif(payload ->> 'channel', ''), nullif(payload ->> 'ref_name', ''), nullif(payload ->> 'first_contact_date', '')::date, nullif(payload ->> 'candidate_folder_url', ''))
  on conflict (candidate_id) do update set name = excluded.name, nickname = excluded.nickname, phone_no = excluded.phone_no, email = excluded.email, doc_group_id = excluded.doc_group_id, group_id = excluded.group_id, channel = excluded.channel, ref_name = excluded.ref_name, first_contact_date = excluded.first_contact_date, candidate_folder_url = excluded.candidate_folder_url;
