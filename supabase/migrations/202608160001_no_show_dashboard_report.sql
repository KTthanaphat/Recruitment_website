-- The company dashboard needs the same operational offer state as the workspace.
...
begin
  ...
  return jsonb_build_object(
    ...
    'requisition_logs', coalesce((select jsonb_agg(jsonb_build_object('log_id', l.log_id, 'doc_id', l.doc_id, 'log_date', l.log_date, 'status', l.status, 'remark', null, 'created_at', l.created_at) order by l.log_date, l.log_id) from public.requisition_logs l where l.status = 'filled'), '[]'::jsonb),
